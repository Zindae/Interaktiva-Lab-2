	...
	this.updateDisplay = function() {
		...
		for (var key in model.menu) {
			if (model.menu[key] !== undefined) {
				id = model.menu[key];
				price = price + model.getDishPrice(id);				//+model.getDish(id).name
				this.dishDiv.append('<div class="col-xs-8 dishname">' +model.getDish(id) +'</div><div class="col-xs-4 dishcost">' + model.getDishPrice(id) + '</div>');
