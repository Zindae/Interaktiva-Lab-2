      ...
      this.updateDisplay = function() {
		if (model.getDish(model.menu['starter']) === undefined || model.getDish(model.menu['main dish']) === undefined || model.getDish(model.menu['dessert']) === undefined)  {
			...
			for(i = 0; i < len; i++){
				...
				$(".dinner-preparation-dish-description").append('<div class="ingr"><b>' + guests*ingredients.quantity+' '+ ingredients.unit+'</b> '+ingredients.name+ ' -- '+ guests*ingredients.price + ' SEK<br></div>');
			};
			...
			var len = main.ingredients.length;
			for(i = 0; i < len; i++){
			...
			var len = dessert.ingredients.length;
			for(i = 0; i < len; i++){
